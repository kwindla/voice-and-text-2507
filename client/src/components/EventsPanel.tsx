...
export function EventsPanel() {
  ...
  return (
    <div className="h-full bg-terminal-black border border-terminal-green-dark rounded-lg p-4 flex flex-col">
      <h3 className="text-sm font-medium text-terminal-green-dark mb-2 flex-shrink-0">RTVI Events</h3>
      <div className="flex-1 overflow-y-auto min-h-0 space-y-1 text-xs font-mono">
        {events.length === 0 ? (
          <div className="text-terminal-green-dark">No events yet...</div>
        ) : (
            ...
            if (!hasMultiple || isExpanded) {
              ...
              return group.events.map((event, index) => (
                <div key={event.id} className="flex items-center gap-2">
                  ...
                  {(!hasMultiple || index > 0) && <span className="text-terminal-green-dark">•</span>}
                  <div className="flex-1 text-terminal-green-dark truncate">
              ...
              return (
                <div key={group.id} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 text-terminal-green-dark">
                    ...
                    {" "}
                    <span className="text-terminal-green-dark">({group.events.length} events)</span>
                  </div>
                </div>
              );
            }
